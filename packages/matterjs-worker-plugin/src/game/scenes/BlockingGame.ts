export class BlockingGame extends Phaser.Scene {
  ...
  constructor() {
    super("BlockingGame"); // Changed scene key

    console.log("BlockingGame constructor"); // Changed log message
  }

  create() {
    console.log("BlockingGame create"); // Changed log message
    ...
    // Add button to switch to ThreadingGame
    const switchToThreadingButton = this.add.text(
      10,
      150,
      "Switch to ThreadingGame",
      {
        font: "16px Arial",
        color: "#000000",
        backgroundColor: "#aaddaa", // Light green
        padding: { x: 10, y: 5 } as Phaser.Types.GameObjects.Text.TextPadding,
      }
    );
    switchToThreadingButton.setScrollFactor(0);
    switchToThreadingButton.setInteractive({ useHandCursor: true });
    switchToThreadingButton.on("pointerdown", () => {
      this.scene.start("ThreadingGame");
    });
